...
if __name__ == "__main__":
    ...
    for type_pattern, clusters in all_clusters.items():
        if type_pattern in unique_mutators.keys():
            print(type_pattern, "has beed analyzed\n")
            continue

        unique_mutators[type_pattern] = []
        print("\n\n================= NEW PATTERN START ==================")
        ...
        for cur_cluster in clusters:
            print("- cur_cluster idx:", cnt_cluster)
            print("- num:", len(cur_cluster))
            print("- members:", cur_cluster)
            print("\n")
            cnt_cluster += 1
        print("==== show cluster end ====\n")

        for cur_cluster in clusters:
            if len(cur_cluster) <= 5:
                groups = [cur_cluster]
            else:
                # group_size = int(len(cur_cluster)/2) + 1
                group_size = 5
                groups = [cur_cluster[i:i + group_size] for i in range(0, len(cur_cluster), group_size)]
            ...
            for group in groups:
                ...
                target_mutators = analysis(group, type_pattern.replace("[", "(").replace("]", ")").replace("'", ""), pattern_description[type_pattern])
                
                if target_mutators:
                    unique_mutators[type_pattern].extend(target_mutators)
                    print("successful")
                else:
                    print("failed")
                    exit(0)
            ...
            with open(res_name, 'w') as json_file:
                json.dump(unique_mutators, json_file)

        print("================= NEW PATTERN END ==================\n\n")
    ...
    #     groups = [mutators[i:i+group_size] for i in range(0, len(mutators), group_size)]

    #     print("mutators:", len(mutators))
    ...
    print("finish")
    print("======= show unique_mutators =======")
    for pattern, mutators in unique_mutators.items():
        print(f" pattern: {pattern} -> num:", len(mutators))
        for mutator in mutators:
            print(f"\t{mutator}")
        print("\n")
